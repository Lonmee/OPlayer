...
export default class ValueMgr implements IMgr {
    ...
    exe(cmd: Cmd) {
        switch (cmd.code) {
            ...
            case 207://"数值操作"
                ...
                // 4:显示信息
                // 5:数值(0)或索引(1)
                let v1 = this.vDic.get(cmd.para[5] == "0" ? parseInt(cmd.para[0]) + 1 : this.vDic.get(cmd.para[0]) + 1);
                let v2 = this.digByType(cmd.para[2], cmd.para[3]);
                this.vDic.set(cmd.para[5] == "0" ? parseInt(cmd.para[0]) + 1: this.vDic.get(cmd.para[0]) + 1, this.calc(v1, v2, cmd.para[1]))
                break;
            case 213://"二周目变量"
                //0：同207数值操作
    ...
    parsePara(p: string, idx: number) {
        return parseInt(p.split("|")[idx]);
    }

    digByType(p1: string, p2: string) {
        switch (parseInt(p1)) {
            case 0:
                return parseInt(p2);
            case 1:
    ...
    digByTag(p: string) {
        switch (p) {
            ...
            default:
                return this.vDic.get(parseInt(p) + 1);
        }
    }
